  ...
  return (
    <div className="flex justify-between px-6 py-1 mt-6 border-b-2 w-11/12 text-left">
      <div className="flex flex-1 items-center">
        <p className="text-sm font-bold">{chainToName(chainId)}</p>{" "}
        <small className="text-xs">({status})</small>
      </div>
      <a
    ...
    <>
      <div className="mx-20 mt-6">
        <p>
          ...
          <span className="text-ceruleanBlue-700">{response?.address}</span> is
          verified
        </p>
      </div>
  ...
  return (
    <>
      <div className="mx-20 mt-6">
        <p>
          The contract at address{" "}
          <span className="text-ceruleanBlue-700">{address}</span> is not
...
const Result = ({ response, setResponse }: ResultProp) => {
  return (
    <div className="flex flex-col basis-0 py-8 flex-grow rounded-lg px-8 transition-all ease-in-out duration-300 bg-white overflow-hidden shadow-md">
      ...
      >
        <path
          d="M22.9168 10.4166L8.3335 25L22.9168 39.5833"
          stroke="black"
          strokeWidth="2"
          strokeLinecap="round"
