...
class Verifier extends React.Component<{}, VerifierState> {
        ...
        if (res.ok && isJson(res)) {
          res.json().then((sessionData: VerifierState) => {
            ...
            if (options.updateDisplayed) {
              const oldVerificationIds = new Set(
                this.state.contracts.map((c) => c.verificationId)
              );
  ...
  private uploadFiles = (files: any[]) => {
    ...
    this.customFetch(ADD_FILES_URL, {
      method: "POST",
      body: formData,
  ...
  private verifyValidated = (sendable: {
  ...
  private displayedContractHandler: ChangeEventHandler<HTMLSelectElement> = (
    e
  ) => {
  ...
  render() {
    ...
    const displayableContracts =
      this.state.displayed === DISPLAY_ALL_CONTRACTS
        ? contracts
        : contracts.filter((c) => c.verificationId === this.state.displayed);
    const unused = this.state.unused;

    const defaultContractOption = (
      <option key={DISPLAY_ALL_CONTRACTS} value={DISPLAY_ALL_CONTRACTS}>
        {" "}
        {DISPLAY_ALL_CONTRACTS} ({this.state.contracts.length}){" "}
      </option>
    );
    const contractOptions = contracts.map((contract) => (
      <option key={contract.verificationId} value={contract.verificationId}>
        {contract.name}
      </option>
    ));
    ...
    return (
      <div
        className="form-container__middle"
      ...
      >
        ...
        <h3>
          ...
          {this.anythingUploaded() && (
            <button
              style={{ float: "right", borderRadius: "5px", cursor: "pointer" }}
        ...
        {contracts && contracts.length > 0 && (
          <div style={{ marginTop: "20px", marginBottom: "20px" }}>
            <span>
              <strong> Contract selection: </strong>
            </span>
            <select
              value={this.state.displayed}
              onChange={this.displayedContractHandler}
            >
              {contractOptions}
            </select>
          </div>
        ...
        {contracts.length === 0 && unused.length > 0 && (
          ...
          >
            {" "}
            No metadata files uploaded{" "}
          </h3>
        )}
        {displayableContracts.length > 0 &&
          displayableContracts.map((contractModel, i) => (
            <Contract
              key={i}
        ...
        {unused.length > 0 && (
          <CollapsableList title="Unused uploads" items={unused} />
        )}
      </div>
    );
  }
