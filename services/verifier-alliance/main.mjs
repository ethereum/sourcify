...
(async () => {
  try {
    ...
    const { rows: verifiedContracts } = await sourceClient.query(
      `
    ...
    for (const contract of verifiedContracts) {
      ...
      const {
        rows: [creationCode],
      } = await sourceClient.query("SELECT * FROM code WHERE code_hash = $1", [
      ...
      await targetClient.query(
        ...
        [
          ...
          verifiedContract.creation_transformations,
          verifiedContract.runtime_match,
          verifiedContract.runtime_values,
          verifiedContract.runtime_transformations,
        ]
      );
    }
