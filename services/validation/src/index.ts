...
function parseRecompilationTarget(input: string) {
  ...
  return {
    path: input.slice(0, delimiterIndex),
    name: input.slice(delimiterIndex + 1),
  };
}
...
function matches(contract: CheckedContract, recompilationTarget: any): boolean {
  ...
  if (typeof recompilationTarget === "string") {
    if (
      recompilationTarget === contract.name ||
      recompilationTarget === contract.compiledPath
    ) {
      return true;
    }
    return false;
...
function logOutput(
  recompilableContracts: CheckedContract[],
  programOptions: any
) {
...
program
  ...
  .description(
    "A tool that validates smart contracts contained in the provided files, directories and zip archives."
  )
  .usage("[options] <path>...")
  .option(
...
if (require.main === module) {
  (async () => {
    ...
    const recompilationTarget = program.prepareJson
      ? parseRecompilationTarget(program.prepareJson)
      : program.prepareJson;
    ...
    const ignoring: any[] = [];
    try {
      checkedContracts = await validationService.checkPaths(
        fileNames,
        ignoring
      );
    } catch (err: any) {
    ...
    checkedContracts.forEach((contract) => {
      if (matches(contract, recompilationTarget)) {
        if (CheckedContract.isValid(contract)) {
          ...
          console.log(
            `The ${OPTION_NAME} target is not a valid contract! Exiting.`
          );
          process.exit(1);
        }
