...
const circleCIWorkflowsYesterdayItem = circleCIWorkflowsJson.items.find(item => {
    const workflowDate = new Date(item.created_at)
    return yesterday.toDateString() === workflowDate.toDateString()
})
assert(circleCIWorkflowsYesterdayItem !== undefined, `There is no backup workflow from yesterday`)

const workflowId = circleCIWorkflowsYesterdayItem.id

// find jobs id of verification-e2e-rinkeby, verification-e2e-sepolia, verification-e2e-goerli
...
const jobIds = circleCIJobsUrlJson.items.filter(job => jobsWithArtifacts.includes(job.name)).map(job => job.id)
...
for (const jobId of jobIds) {
    ...
    const circleCIArtifactVerifiedContractResult = await fetch(circleCIArtifactVerifiedContractUrl)
    const circleCIArtifactVerifiedContractJson = await circleCIArtifactVerifiedContractResult.json()
    const { deploymentAddress, deploymentChain } = circleCIArtifactVerifiedContractJson
    if (deploymentAddress && deploymentChain) {
        try {
            const s3Object = await bareBonesS3.send(new GetObjectCommand({
            ...
            console.log(`not in backup: stable/repository/contracts/full_match/${deploymentChain}/${deploymentAddress}/metadata.json`)
        }
    }
}
